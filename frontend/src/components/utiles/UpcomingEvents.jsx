...
const UpcomingEvents = ({ upcomingEvents = [], upcomingRegEvents }) => {
  ...
  const displayedRegEvents = viewAll
    ? sortedUpcomingRegEvents // Changed from showAll to viewAll here
      ...
      <section className="grid gap-4 w-full">
        <div className="w-full flex items-center justify-between gap-2">
          ...
          {sortedUpcomingRegEvents.length > 2 && (
            <p
              className="flex items-center gap-2 text-[#767676] text-base cursor-pointer font-medium"
              onClick={() => setViewAll(!viewAll)} // Changed from setShowAll to setViewAll here
            >
        ...
        {displayedRegEvents.length > 0 ? ( // Changed from displayedEvents to displayedRegEvents
